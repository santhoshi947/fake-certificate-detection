...
# ---------------------------------------------------------------------------
# Ordered list of feature columns EXACTLY matching the training dataset.
# The model's predict() requires this exact order.
# ---------------------------------------------------------------------------
MODEL_FEATURE_COLUMNS = [
    'edge_count', 'edge_density', 'contour_count', 'image_sharpness',
    'noise_level', 'blur_score', 'brightness', 'contrast', 'hist_peak',
    'hist_spread', 'logo_match_score', 'logo_area', 'logo_mean_intensity',
    'logo_std_intensity', 'logo_edge_density', 'seal_circularity',
    'seal_circularity_std', 'seal_area', 'seal_count', 'max_seal_area',
    'seal_match_score', 'signature_edge_density', 'signature_pixel_mean',
    'signature_pixel_std', 'signature_contour_count', 'signature_area',
    'signature_aspect_ratio', 'signature_match_score', 'extracted_text_length',
    'word_count', 'ocr_avg_confidence', 'ocr_min_confidence',
    'ocr_std_confidence', 'char_height_mean', 'char_height_std',
    'char_height_variance', 'char_height_range', 'char_width_mean',
    'char_width_std', 'char_width_variance', 'char_spacing_mean',
    'char_spacing_std', 'char_spacing_variance', 'char_spacing_range',
    'line_spacing_mean', 'line_spacing_std', 'line_spacing_variance',
    'text_density', 'stroke_density', 'text_region_sharpness', 'digit_count',
    'letter_count', 'uppercase_count', 'space_count', 'digit_ratio',
    'uppercase_ratio', 'usn_count', 'usn_valid', 'suspicious_usn_pattern',
    'date_count', 'date_valid', 'score_count', 'score_mean', 'score_std',
    'score_validity_ratio', 'grade_count', 'name_count', 'avg_name_length',
    'table_structure_score', 'line_length_mean', 'line_length_std',
    'line_length_variance', 'image_height', 'image_width', 'aspect_ratio',
]
...
class CertificateFeatureExtractor:
    ...
    def _extract_logo_features(self, gray: np.ndarray) -> Dict[str, float]:
        """Extract logo-related features.
        
        If a logo template is provided, uses OpenCV template matching.
        Otherwise analyses the header region for graphic (non-text) content
        by morphologically suppressing text strokes and measuring the density
        of remaining compact graphic blobs (emblems / seals / logos).
        """
        features = {}

        try:
            if self.logo_template is not None:
                # ── A. Template-based matching ─────────────────────────────
                result = cv2.matchTemplate(gray, self.logo_template, cv2.TM_CCOEFF_NORMED)
                _, max_val, _, max_loc = cv2.minMaxLoc(result)
                features['logo_match_score'] = float(np.clip(max_val, 0.0, 1.0))
                ...
                if max_val > 0.5:
                    ...
                    features['logo_edge_density'] = float(
                        np.sum(logo_edges > 0) / max(h * w, 1))
                else:
                    features.update({'logo_area': 0, 'logo_mean_intensity': 0,
                                     'logo_std_intensity': 0, 'logo_edge_density': 0})
            else:
                # ── B. No template — structural graphic analysis ────────────
                # Strategy:
                #   1. Take the top 22% of the image (header).
                #   2. Binarise and remove TEXT-like connected components:
                #      text chars are narrow & tall; logos are compact/circular.
                #   3. The remaining graphic blobs indicate a real logo/emblem.
                #   4. Additionally run HoughCircles for circular emblems.

                h_img, w_img = gray.shape
                header = gray[:int(h_img * 0.22), :]
                hh, hw = header.shape

                # Binarise
                _, bw = cv2.threshold(header, 0, 255,
                                      cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)

                # ── Remove text-like CCs: keep only LARGE graphic blobs ────
                # A typical letter in a certificate header is ~0.0001-0.0005 of header area.
                # A logo emblem is significantly larger: we require ≥ 0.002 of header area
                # (about 4–5× the typical letter character size).
                n_labels, labels, stats, _ = cv2.connectedComponentsWithStats(
                    bw, connectivity=8)
                graphic_mask = np.zeros_like(bw)
                graphic_blob_count = 0
                # minimum pixel area for a blob to be considered a logo element
                min_logo_px = hh * hw * 0.002       # ~4× larger than a typical letter
                for lbl in range(1, n_labels):
                    cw  = int(stats[lbl, cv2.CC_STAT_WIDTH])
                    ch  = int(stats[lbl, cv2.CC_STAT_HEIGHT])
                    ca  = int(stats[lbl, cv2.CC_STAT_AREA])
                    if ca < min_logo_px:        # too small — probably a letter or noise
                        continue
                    aspect = min(cw, ch) / max(max(cw, ch), 1)
                    if aspect > 0.28:           # compact enough to be a logo element
                        graphic_mask[labels == lbl] = 255
                        graphic_blob_count += 1

                # Graphic pixel density in header (excluding text)
                graphic_density = float(
                    np.sum(graphic_mask > 0) / max(hh * hw, 1))

                # ── Circle detection (institutional logos are often circular) ──
                blurred = cv2.GaussianBlur(header, (7, 7), 2)
                circles = cv2.HoughCircles(
                    blurred, cv2.HOUGH_GRADIENT, dp=1.2,
                    minDist=hw // 4, param1=50, param2=25,
                    minRadius=int(min(hh, hw) * 0.05),
                    maxRadius=int(min(hh, hw) * 0.48),
                )
                circle_count = len(circles[0]) if circles is not None else 0

                # ── Score combination ───────────────────────────────────────
                # graphic_density:    0 = no logo blobs  /  0.03+ = logo present
                # graphic_blob_count: 0 = no distinct shapes / 5+ = likely logo
                # circle_count:       circles strongly indicate an emblem

                graphic_score = min(graphic_density / 0.025, 1.0)
                blob_score    = min(graphic_blob_count / 8.0, 1.0)
                circle_bonus  = min(circle_count * 0.25, 0.40)

                raw_score = graphic_score * 0.50 + blob_score * 0.30 + circle_bonus

                # Hard penalise if virtually no compact graphic blobs found
                if graphic_blob_count == 0:
                    raw_score = min(raw_score, 0.15)

                logo_score = float(np.clip(raw_score, 0.0, 1.0))

                features['logo_match_score']    = logo_score
                features['logo_area']           = float(graphic_blob_count)
                features['logo_mean_intensity'] = float(np.mean(header))
                features['logo_std_intensity']  = float(np.std(header))
                features['logo_edge_density']   = graphic_density
    ...
    def _extract_seal_features(self, gray: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            # Template matching if seal template prov2ided
    ...
    def _extract_format_validation_features(self, gray: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            # USN / Enrollment number validation
            ...
            # Date validation — supports many real-world certificate date formats:
            #  DD/MM/YYYY  DD-MM-YYYY  YYYY/MM/DD
            #  12 Feb 2025  Feb 12, 2025  February 12, 2025
            #  12th February 2025  12th Feb 2025
            _MONTHS = (
            ...
            date_patterns = [
                ...
                # Numeric: YYYY/MM/DD, YYYY-MM-DD
                r'\b\d{4}[/\-\.]\d{1,2}[/\-\.]\d{1,2}\b',
                # DD Month YYYY or DDth Month YYYY  (12 February 2025 / 12th Feb 2025)
                r'\b\d{1,2}(?:st|nd|rd|th)?\s+(?:' + _MONTHS + r')\.?\s+\d{2,4}\b',
                # Month DD, YYYY or Month DD YYYY  (February 12, 2025 / Feb 12 2025)
                r'\b(?:' + _MONTHS + r')\.?\s+\d{1,2}(?:st|nd|rd|th)?,?\s+\d{2,4}\b',
                # Month YYYY — partial dates that still confirm issuance year
...
# ============================================================
# PREDICTION WRAPPER — returns (feature_vector, feature_dict)
# ============================================================
...
def extract_features_for_prediction(
    ...
    """
    Extract features and return both the numpy vector (for model.predict)
    and the full named dict (for threshold-based explanation).

    The vector columns are aligned to MODEL_FEATURE_COLUMNS, so missing
    values are filled with 0.0 to prevent shape errors.

    Args:
        image_path: Path to certificate image file.
        logo_template: Optional path to genuine logo template.
        seal_template: Optional path to genuine seal template.
        signature_template: Optional path to genuine signature template.

    Returns:
        (feature_vector, feature_dict)
        - feature_vector : np.ndarray of shape (1, n_features)
        - feature_dict   : dict of all extracted feature values
    """
    ...
    # Build the ordered vector, filling any missing column with 0.0
    ordered_values = [
        float(feature_dict.get(col, 0.0)) for col in MODEL_FEATURE_COLUMNS
    ]
    feature_vector = np.array(ordered_values, dtype=np.float64).reshape(1, -1)
...
if __name__ == "__main__":
    ...
    image_path = r"C:\Users\HP\OneDrive\Desktop\certificate_detector\data\genuine\gen2.jpg" # Change this!
